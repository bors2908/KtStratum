...
import static ge.becrin.kt.stratum.message.Message.JSON_STRATUM_KEY_ID;
...
public class MessageMarshaller {
    ...
    protected Map<String, Class<? extends NotificationMessage>> notificationMethodMap;
    ...
    public MessageMarshaller() {
        ...
        this.notificationMethodMap = new HashMap<>();
    }
     ...
     *
     * @param methodName The name of the method, as it appears in requests.
     * @param messageType The type of concrete message type for the method.
     */
    public void registerRequestHandler(final String methodName, final Class<? extends RequestMessage> messageType) {
        this.requestMethodMap.put(methodName, messageType);
    }

    public void registerNotificationHandler(final String methodName, final Class<? extends NotificationMessage> messageType) {
        this.notificationMethodMap.put(methodName, messageType);
    }

    /**
     * Gets the appropriate concrete message type to handle the specified Stratum method.
     *
     * @param methodName The name of the method for which a message type is desired.
     *
    ...
    public Class<? extends Message> getNotificationHandlerForMethod(final String methodName) {
        return this.notificationMethodMap.get(methodName);
    }
     ...
     * @param responseType The type of response message expected from the request.
     *
     * @throws IllegalArgumentException If {@code requestId} matches a request that is already registered as pending with this
     * instance.
     */
    @SuppressWarnings("UnstableApiUsage")
     ...
     * messages.
     *
     * @param jsonString The string of JSON data.
     *
     * @return The list of one or more concrete {@link Message}s that represent the information from
     * the provided JSON message.
     *
     * @throws MalformedStratumMessageException If any JSON given is not a properly-formed Stratum message.
     * @throws UnhandledStratumMessageException If any message given doesn't match any of the types of messages that this marshaller was
     ...
     *
     * @param jsonMessages The JSON array containing each message as a JSONObject.
     *
     ...
     *
     * @param jsonMessage The JSON object containing the full message.
     *
     * @return A concrete {@link Message} that represents the information from the provided JSON
     * message.
     *
     * @throws MalformedStratumMessageException If the JSON given is not a properly-formed Stratum message.
     * @throws UnhandledStratumMessageException If the message given doesn't match any of the types of messages that this marshaller was
     * expecting or is capable of marshalling.
     */
    public Message marshalMessage(final JSONObject jsonMessage)
        ...
        if (!jsonMessage.has(JSON_STRATUM_KEY_ID)) {
            result = marshalNotificationMessage(jsonMessage);
        } else if (jsonMessage.has(JSON_MESSAGE_KEY_RESULT)) {
     ...
     *
     * @param message The Stratum message to unmarshal.
     *
     ...
     *
     * @param jsonMessage The JSON object containing the full request message.
     *
    ...
    protected Message marshalRequestMessage(final JSONObject jsonMessage)
        ...
        if (requestType == null) {
            //TODO Why it was throwing??
            // throw new UnhandledStratumMessageException(request);
            result = this.marshalMessage(jsonMessage, RequestMessage.class);
        } else {
     ...
     *
     * @param jsonMessage The JSON object containing the full response message.
     *
     * @return A concrete {@link Message} that represents the response information from the provided
     * JSON message.
     *
     * @throws MalformedStratumMessageException If the JSON information is invalid or doesn't match any of the responses that this marshaller
    ...
    protected Message marshalNotificationMessage(final JSONObject jsonMessage)
        throws MalformedStratumMessageException, UnhandledStratumMessageException {
        final Message result;
        final NotificationMessage request = new NotificationMessage(jsonMessage);
        final String methodName = request.getMethodName();
        final Class<? extends Message> requestType = this.notificationMethodMap.get(methodName);

        if (requestType == null) {
            //TODO Why it was throwing??
            // throw new UnhandledStratumMessageException(request);
            result = this.marshalMessage(jsonMessage, NotificationMessage.class);
        } else {
            result = this.marshalMessage(jsonMessage, requestType);
        }
     ...
     *
     * @param jsonMessage The JSON object containing the message.
     * @param messageType The type of message into which the JSON data will be marshalled.
     *
     * @return A concrete {@link Message} that represents the information from the provided JSON
     * message.
     *
     * @throws MalformedStratumMessageException If the JSON information is invalid.
     */
     ...
     * @param expectedResponseType The type of response that was expected for the message.
     */
