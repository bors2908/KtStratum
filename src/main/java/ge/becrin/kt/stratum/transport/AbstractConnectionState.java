...
public abstract class AbstractConnectionState
    ...
    private final Map<Class<? extends NotificationMessage>, MessageListener<NotificationMessage>> notificationHandlers;
     ...
     *
     * @param transport The Stratum message transport to which the state corresponds.
     */
    ...
    public void start() {
        ...
        transport.registerNotificationListener(this.notificationListener);
    }
    ...
    public void end() {
        ...
        transport.unregisterNotificationListener(this.notificationListener);
    }
     ...
     * @param messageType The type of message for which a handler is being registered.
     * @param handler The handler to invoke for the message.
     *
     * @throws IllegalArgumentException If a handler for the method specified in {@code methodName} is already registered.
     */
     ...
     * @param messageType The type of message for which a handler is being registered.
     * @param handler The handler to invoke for the message.
     * @param replace Whether or not to replace any existing handlers for the method specified in
     * {@code methodName}.
     *
     * @throws IllegalArgumentException If a handler for the method specified in {@code methodName} is already registered and
    ...
    protected <T extends NotificationMessage> void registerNotificationHandler(
        final String methodName,
        final Class<T> messageType,
        final MessageListener<T> handler)
        throws IllegalArgumentException {
        this.registerNotificationHandler(methodName, messageType, handler, false);
    }
    ...
    protected <T extends NotificationMessage> void registerNotificationHandler(
        ...
        if (!replace && this.notificationHandlers.containsKey(messageType)) {
            throw new IllegalArgumentException(
                "A handler is already registered for this request message type: " + messageType.getName()
            );
        }

        marshaller.registerNotificationHandler(methodName, messageType);
        this.notificationHandlers.put(messageType, (MessageListener<NotificationMessage>)handler);
    }
    ...
    @Override
    public boolean processNotification(final NotificationMessage message) {
        boolean handled = false;
        final MessageListener<NotificationMessage> handler = this.notificationHandlers.get(message.getClass());

        if (handler == null) {
            if (LOGGER.isErrorEnabled()) {
                LOGGER.error(
                    "Notification message '{}' was ignored by state '{}'.",
                    message.getClass().getName(),
                    this.getClass().getName()
                );
            }
     ...
     *
     * @param newState The new state for the connection.
     */
     ...
     * @author Guy Paddock (guy@inveniem.com)
     */
    protected class RequestListener implements MessageListener<RequestMessage> {
        @Override
     ...
     * @author Guy Paddock (guy@inveniem.com)
     */
    protected class ResponseListener implements MessageListener<ResponseMessage> {
        @Override
    ...
    protected class NotificationListener implements MessageListener<NotificationMessage> {
        @Override
        public void onMessageReceived(final NotificationMessage message) {
            AbstractConnectionState.this.processNotification(message);
        }
    }
}
