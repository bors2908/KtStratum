...
public abstract class AbstractTcpMessageTransport
     ...
     *
     * @throws IllegalStateException If the client is not currently connected.
     */
    @Override
     ...
     *
     * @param message The request message to send.
     * @param responseType The type of response expected for the request. This may be {@code null}.
     *
     * @throws IllegalStateException If the connection is not currently open.
     */
    @Override
    public void sendRequest(final RequestMessage message,
    ...
    protected void handleUnsupportedRequest(final RequestMessage request) {
        final ResponseMessage response = new UnsupportedMethodResponse(request.getId(), request.getMethodName());

        this.sendResponse(response);
    }

    protected void handleUnsupportedNotification(final NotificationMessage notification) {
        final ResponseMessage response = new UnsupportedMethodResponse(null, notification.getMethodName());
