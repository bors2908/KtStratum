...
public class TcpSocketInputThread
     ...
     *
     * @param transport The transport to which the input thread pertains.
     */
                ...
                if (!jsonLine.isEmpty()) {
                    synchronized (this.transport) {
                        ...
                        try {
                            ...
                            if (ex.getUnhandledRequest() != null) {
                                this.transport.handleUnsupportedRequest(ex.getUnhandledRequest());
                            } else if (ex.getUnhandledNotification() != null) {
                                this.transport.handleUnsupportedNotification(ex.getUnhandledNotification());
                            }
                        }
                    }
                }
